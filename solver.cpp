...
public:
    ...
    const double gamma = 1.4; // specific heat ratio for ideal gas
        ...
        :Ma(Ma), N(N), dt(dt), t_final(t_final), sample_interval(sample_interval)
    {
        ...
        p_init = 1.0 / (gamma * Ma * Ma); // initial pressure based on ideal gas law
    }
};
...
void compute_tentative_velocity(const StateVector& U, StateVector& U_tmp, const Parameters& params, double t_current) {
    ...
    solver.compute(J);
    if (solver.info() != Eigen::Success) {
        throw std::runtime_error("Decomposition failed in Newton iteration.");
    }
    U_diff = solver.solve(-Residual);
    if (solver.info() != Eigen::Success) {
...
Eigen::SparseMatrix<double> build_augmented_Laplacian(const Parameters& params) {
    ...
    std::cout << "zero-row-sum of Laplacian: " << norm << std::endl;
    if (norm > 1e-8) {
